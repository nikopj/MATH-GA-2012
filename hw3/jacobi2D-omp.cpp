...
double* jacobi(double* u, double* f, long N, long* k, long maxit, double tol) {
    double* u_new = (double*) malloc(N * N * sizeof(double));
    #pragma omp parallel for
    for (long n = 0; n < N*N; n++) u_new[n] = 0.0;
    ...
    for (*k = 0; *k < maxit; *k += 1) {
        ...
        for (long n = N+1; n < N*(N-1); n++) {
            long i =  n / N;
            long j =  n % N;
            if (i > 0 && j > 0 && i < N-1 && j < N-1) {
                double u_new_ij = 0.25 * (h2*f[n] + u[n-N] + u[n-1] + u[n+N] + u[n+1]);
                diff = fmax(diff, fabs(u_new_ij - u[n]));
                u_new[n] = u_new_ij;
            }
        }
        // swap u and u_new pointers
        double* temp = u;
        u = u_new;
        u_new = temp;
